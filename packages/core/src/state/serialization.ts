...
export function serializeBaton(b: Baton): Record<string, unknown> {
  return {
    ...
    tried_and_rejected: b.triedAndRejected,
    open_questions: b.openQuestions,
    next_steps: b.nextSteps,
...
export function deserializeBaton(d: Record<string, unknown>): Baton {
  return {
    ...
    decisions: (d["decisions"] as Baton["decisions"]) ?? [],
    triedAndRejected: (d["tried_and_rejected"] as Baton["triedAndRejected"]) ?? [],
